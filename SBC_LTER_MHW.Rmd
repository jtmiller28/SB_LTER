...
nci_temp_sep <- nci_temp%>% 
  separate(datetime_gmt, c("date", "time"), " ") #We need to seperate out the data so that times are seperated from the data
```
...
nci_temp_14m_avgT <- nci_temp_14m %>% 
  group_by(date, site) %>% 
  mutate(mean_daily_temp = mean(temperature_c))
...
nci_temp_14m_avgT_daily <- select(nci_temp_14m_avgT_daily, !time)
```
